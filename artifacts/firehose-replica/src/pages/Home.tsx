...
const CUSTOM_APPS = [
  {
    ...
    industry: "HOME SERVICES",
    title: "Quote Builder",
    description: "A branded estimating tool that pulls live material costs, applies your labor rates by job type, and generates a professional proposal in under two minutes. Estimators stop copy-pasting from spreadsheets. Quotes go out the same day the lead comes in — not three days later.",
  },
  {
    icon: <Target className="h-6 w-6" />,
    industry: "FIELD OPERATIONS",
    title: "Dispatch Board",
    description: "A real-time scheduling dashboard that shows every crew, every job, and every open slot in a single view. Drag-and-drop assignment, automatic SMS notifications to techs when jobs are assigned or changed, and a live map view of where your field team is at any given moment.",
  },
  {
    icon: <Brain className="h-6 w-6" />,
    industry: "PROFESSIONAL SERVICES",
    title: "Client Portal",
    description: "A branded, login-gated portal where your clients can check job status, view documents, approve estimates, and message your team — without ever touching your internal tools. Kills the \"where are we on this?\" email thread entirely and makes your operation look enterprise-grade.",
  },
];
  ...
  return (
    <section className="relative overflow-hidden border-b-4 border-black bg-[hsl(46,100%,96%)]">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute top-6 right-6 h-20 w-20 border-4 border-black bg-[hsl(47,100%,50%)] opacity-30 hidden sm:block" />
        <div className="absolute bottom-6 left-6 h-16 w-16 rotate-12 border-4 border-black bg-[hsl(46,50%,88%)] opacity-30 hidden sm:block" />
      </div>
  ...
  return (
    <section className="relative border-b-4 border-black bg-[hsl(46,100%,96%)]">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-24">
        ...
        <p className="mt-4 max-w-3xl text-xl font-bold opacity-60">
          Once we know what's broken, we build what fixes it. The specific form depends on your business. The outcome is always the same.
        </p>

        <div className="mt-12 grid gap-6 lg:grid-cols-2">

          {/* AI Agents card */}
          <div className="flex flex-col border-4 border-black bg-[hsl(47,100%,50%)] p-8 shadow-neo-lg">
            <div className="mb-5 inline-block self-start border-4 border-black bg-black px-3 py-1 text-xs font-bold tracking-widest uppercase text-[hsl(47,100%,50%)]">
              AI Agents
            </div>
            <p className="mb-6 text-sm font-bold leading-relaxed text-black/60">
              Agents watch, learn, decide, and act across whatever data matters most in your business — cost, pipeline, relationships, risk, capacity. The specific trigger is yours to define. The outcome is always the same: you stop finding out about problems after they've already cost you money.
            </p>
            <div className="flex flex-col gap-2">
              {[
                { title: "Cost & Margin Intelligence", body: "Monitors costs across suppliers, job types, or vendors — spots drift and surfaces the signal before it hits your P&L." },
                { title: "Pre-Decision Risk Scoring", body: "Scores new decisions against your historical outcomes. Before you commit resources, you get a probability — not a gut feeling." },
                { title: "Institutional Memory", body: "Ingests recordings, emails, notes, SOPs, and CRM data — makes it queryable by anyone on your team in seconds." },
                { title: "Pipeline & Relationship Health", body: "Reads the signals buried in your communication patterns and builds a live health score for every relationship or opportunity." },
              ].map((sub) => (
                <div key={sub.title} className="border-2 border-black bg-black/10 p-4">
                  <p className="mb-1 text-xs font-extrabold uppercase tracking-widest text-black">{sub.title}</p>
                  <p className="text-sm font-bold leading-snug text-black/55">{sub.body}</p>
                </div>
              ))}
              <div className="border-2 border-dashed border-black/40 p-4">
                <p className="text-xs font-extrabold uppercase tracking-widest text-black/40">+ And More</p>
                <p className="mt-0.5 text-sm font-bold text-black/35">If the data exists in your business, we can build an agent around it.</p>
              </div>
            </div>
          </div>

          {/* Custom Apps card */}
          <div className="flex flex-col border-4 border-black bg-white p-8 shadow-neo-lg">
            <div className="mb-5 inline-block self-start border-4 border-black bg-black px-3 py-1 text-xs font-bold tracking-widest uppercase text-[hsl(47,100%,50%)]">
              Custom Applications
            </div>
            <p className="mb-6 text-sm font-bold leading-relaxed opacity-55">
              We build the tool that actually fits the way your business runs. Not configured from a template. Not a workflow bolted onto software built for someone else. Built from scratch around your process, your data, your team.
            </p>
            <div className="flex flex-col gap-2">
              {[
                { title: "Estimation & Proposal Tools", body: "Wired into your real costs and margin logic. Outputs a professional deliverable in under two minutes. Your process, automated." },
                { title: "Operations & Scheduling Dashboards", body: "A real-time view of whatever your team needs to coordinate — jobs, crews, assets, capacity. Built around how your operation actually runs." },
                { title: "Client & Stakeholder Portals", body: "A branded, login-gated environment where clients see exactly what you want them to see. Eliminates the 'where are we on this?' thread entirely." },
              ].map((sub) => (
                <div key={sub.title} className="border-2 border-black bg-[hsl(46,100%,96%)] p-4">
                  <p className="mb-1 text-xs font-extrabold uppercase tracking-widest text-black">{sub.title}</p>
                  <p className="text-sm font-bold leading-snug opacity-50">{sub.body}</p>
                </div>
              ))}
              <div className="border-2 border-dashed border-black/30 p-4">
                <p className="text-xs font-extrabold uppercase tracking-widest text-black/35">+ And More</p>
                <p className="mt-0.5 text-sm font-bold opacity-30">If the data exists in your business, we can build a custom application around it.</p>
              </div>
            </div>
          </div>
...
function PricingModel() {
  const tier1 = [
    ...
    "3 custom AI agent or app buildouts per year",
  ...
  const tier2 = [
    ...
    "12 custom AI agent or app buildouts per year",
  ...
  const tier3SharedFeatures = [
    ...
    "Full cross-org operations mapping available",
  ];
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3 items-start">
          ...
          <div className="relative flex flex-col border-4 border-black bg-white p-8 shadow-neo-lg">
            ...
            <div className="mt-8 border-t-4 border-black pt-5">
              ...
              <p className="mt-1 text-sm font-bold">$10,000/year - save ~$2,000. No setup fee.</p>
            </div>
          </div>
          ...
          <div className="relative flex flex-col border-4 border-black bg-black p-8 shadow-neo-lg">
            ...
            <div className="mt-6 flex-1 space-y-4">
              <div className="border-2 border-white/20 p-4">
                <p className="text-xs font-extrabold uppercase tracking-widest text-[hsl(47,100%,50%)] mb-1">À La Carte Build-Outs</p>
              ...
              <div className="border-2 border-white/20 p-4">
                <p className="text-xs font-extrabold uppercase tracking-widest text-[hsl(47,100%,50%)] mb-1">Short-Term Contract Engagements</p>
...
function VCRightPanel() {
  return (
    <div className="flex flex-col border-4 border-black bg-[hsl(47,100%,50%)] p-6 shadow-neo-md lg:p-10">
      ...
      <div className="mb-3 border-4 border-black bg-black/10 p-4 lg:mb-4 lg:p-5">
        <VCBlockTag onDark>Custom Agent Builds — Level 3</VCBlockTag>
        <div className="flex flex-col gap-2">
          {vcAgents.map((item, i) => (
            <div key={i} className="flex items-start gap-3">
              <VCCheckMark onDark />
      ...
      <div className="mt-auto border-4 border-black bg-black p-4 lg:p-5">
        ...
        <p className="mt-2 text-[11px] font-bold uppercase tracking-wide text-white/50">
          No Setup Fee. One Team Owns All Of It.
        </p>
      </div>
    </div>
  );
